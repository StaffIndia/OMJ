	...
	if ($this->form_validation->run() == FALSE)
                {
					...
					$query1=$this->db->where('PID = 0');
						$query1 =$this->db->where('((MSGTO = '.$id.' AND MSGFROM='.$userid.') OR (MSGTO = '.$userid.' AND MSGFROM='.$id.'))');
	...
	if ($this->form_validation->run() == FALSE)
                {
					...
					$sdata['id']=$this->uri->segment(3);
					$sdata['pid']= $this->uri->segment(4);
					$userid=$this->session->userdata('userid');
					
			
					$query1=$this->db->select('*,a.fullname as auser,b.fullname buser,b.profilepicture as bpic,inbox.time as mtime');
					$query1=$this->db->from('inbox');
					$query1=$this->db->where('inbox.PID = '.$sdata['pid'].'');
					$query1=$this->db->join('members a', 'a.USERID = inbox.MSGTO');
					$query1=$this->db->join('members b', 'b.USERID = inbox.MSGFROM');
					$query1=$this->db->join('files', 'files.FID = inbox.FID','left');
					$query1 =$this->db->where('((MSGTO = '.$id.' AND MSGFROM='.$userid.') OR (MSGTO = '.$userid.' AND MSGFROM='.$id.'))');
					...
					$this->load->library('pagination');
					$config['base_url'] = site_url('conversation/user/'.$id.'/'.$pid.'');
					 $config['total_rows'] = $query1;
					 $config['per_page'] = 10;
					 $config['num_links'] = 5;
					 $config['full_tag_open'] = '<ul class="pagination no-margin">';
					...
					$this->load->view('inc/header');
					$this->load->view('inc/navbar');
					$this->load->view('conversation-project',$sdata);
					$this->load->view('inc/footer');
                }
                else
                {
					...
					//user select file
					  ...
					  $this->load->library('upload', $config);
					   ...
					   $msg2=' <div class="alert alert-danger">'.$msg1.'</div>';
						 $this->session->set_flashdata('message',$msg2 );
						redirect('conversation/project/'.$id.'/'.$data['PID'].'');
